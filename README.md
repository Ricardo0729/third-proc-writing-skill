...
> Agent Skill — 起草、修改、润色检察机关内部公文。面向第三检察部（经济犯罪检察）常见工作材料。
...
支持 **11 类检察机关常用工作文稿** 的起草、修改与润色；提示函和简版模板作为相应类型的变体：
...
| ✉️ 函 | 平级单位商洽工作 |
| 📣 通知 | 传达事项、部署工作并明确办理要求 |
| 📊 质效分析 | 梳理业务数据、运行态势、问题及改进方向 |
| 🔍 案件分析材料 | 类案或个案综合分析 |

案件材料支持事实、程序状态、争议观点及来源整理；不代替承办人作个案结论或办案决定。详见 `references/风险规则.md`。
...
技能正文使用通用 `SKILL.md` 结构。各 Agent 的目录、元数据识别、安装命令和权限行为可能不同；以下命令仅供参考，使用前请核对对应平台的当前官方说明。安装成功不等于平台已按预期加载所有参考文件。
...
| 理念 | 说明 |
|------|------|
| **来源清楚** | 区分材料事实、主张、意见和未知事项；冲突不静默合并 |
| **缺项透明** | 不补造事实；使用说明具体缺项的占位符 |
| **按需确认** | 简单明确的任务直接交付；复杂、多义或用户要求时先确认提纲 |
| **核查有边界** | 只报告实际检查过的项目，不把自查冒充正式审查 |
| **风险规则** | R1-R9 以 `references/风险规则.md` 为唯一编号来源 |
...
1. 阅读 `references/快速入门指南.md`，了解如何说明文种和用途。
2. 查看 `references/文种速查表.md`，定位适用文种。
3. 按当前任务读取对应模板与文种规则。
4. 按需参考 `references/常用表达.md`。
5. 选择 `references/自查清单.md` 中适用的项目，不要求每份材料逐项输出全部清单。

## 🛡️ 风险边界（R1-R9）

| 编号 | 规则 | 说明 |
|------|------|------|
| R1-R9 | 事实和数据来源、法律依据、案件判断、敏感材料、客观表述、个案意见及个人风格边界 | 以 `references/风险规则.md` 为准，避免在 README 重复维护规则正文 |
